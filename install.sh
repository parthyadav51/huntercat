...
echo "For Mac and Windows, refer to the official guide https://huntercat.wiki"
echo "#########################################################################"

echo " "
tput setaf 4;
echo "Installing huntercat and its dependencies"

echo " "
if [ "$EUID" -ne 0 ]
  then
  tput setaf 1; echo "Error installing huntercat, Please run this script as root!"
...
tput setaf 4;
echo "#########################################################################"
echo "Installing huntercat"
echo "#########################################################################"
make certs && make build && make up && tput setaf 2 && echo "huntercat is installed!!!" && failed=0 || failed=1
