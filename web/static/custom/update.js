...
function check_huntercat_update() {
  if (
    ...
    window.open("https://github.com/yogeshojha/huntercat/releases", "_blank");
  } else {
    Swal.fire({
      title: "Checking huntercat latest version...",
      ...
      .then(function (response) {
        ...
        if (response["description"] == "RateLimited") {
          ...
          Swal.fire({
            ...
            text: "You are running the latest version of huntercat!",
            icon: "info",
          });
        }
      });
...
function update_available(latest_version_number, changelog) {
  ...
  Promise.all([
    ...
    Swal.fire({
      ...
      html: `
          ${changelogStyle}
          <h5>huntercat's new update ${latest_version_number} is available, please follow the update instructions.</h5>
      ...
      if (result.isConfirmed) {
        window.open("https://www.huntercat.wiki/update", "_blank");
      }
    });
  });
}
...
function showAfterUpdatePopup() {
  ...
  const currentVersion = document.body.getAttribute("data-huntercat-version");
  ...
  if (lastShownVersion !== currentVersion) {
    ...
    // Swal.fire({
    //   title: isFirstRun ? "Welcome to huntercat!" : "Thanks for updating!",
    ...
    // }).then((result) => {
    //   if (result.isConfirmed) {
    //     window.open("https://huntercat.wiki/changelog/latest", "_blank");
    //   }
    //   localStorage.setItem("lastShownUpdateVersion", currentVersion);
    // });
    Swal.fire({
      title: "Thanks for using huntercat!",
      ...
      if (result.isConfirmed) {
        window.open(`https://huntercat.wiki/whats-new/${currentVersion.replace(/\./g, "_")}`, "_blank");
      }
      localStorage.setItem("lastShownUpdateVersion", currentVersion);
    });
  }
