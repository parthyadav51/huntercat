...
from huntercat.common_func import create_inappnotification, get_hackerone_key_username
from huntercat.definitions import PROJECT_LEVEL_NOTIFICATION, HACKERONE_ALLOWED_ASSET_TYPES
from huntercat.celery import app
from huntercat.database_utils import bulk_import_targets
...
def import_hackerone_programs_task(handles, project_slug, is_sync = False):
	...
	for handle in handles:
		...
		if program_details:
			...
			# via PR https://github.com/yogeshojha/huntercat/pull/1410
			try:
				...
				for scope in scopes:
					...
					# we need to filter the scope that are supported by huntercat now
